...
**Added:** `info.role` template metadata.

**Reason:** Engine behaviour (schema collapse, field-level lineage, structural classification) was keyed to hardcoded template names, so a pack template like `AvroSchema extends Schema` got none of the engine's behaviour (2026-07-02 principal engineer review, C1/P2.2). Roles make the capability contract explicit and declared once.

**How it works:** a template may declare `info.role` as one of `field`, `value`, `type-container`, `enum-container`, or `mapping`. Roles resolve through the `extends` chain (nearest declaration wins), so subclasses inherit engine behaviour. Core pack templates declare: Field=`field`, EnumValue=`value`, Schema=`type-container`, EnumDefinition=`enum-container`, Mapping=`mapping`. Structural containment edges are generated by role (`field` → `has-field`, `value` → `has-value`), and section names in cluster files are fully pack-defined — the engine never matches on section-name or template-name literals.
